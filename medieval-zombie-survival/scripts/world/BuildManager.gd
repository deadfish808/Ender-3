...
func enter_build(item_id: String) -> void:
	...
	if cfg.get("frames", 0) > 0:
		...
		_ghost.texture = at
		_ghost.offset = Vector2(0, 32.0 - fs.y / 2.0)
	else:
		_ghost.texture = load(cfg["texture"])
		_ghost.offset = Vector2(0, 32.0 - _ghost.texture.get_height() / 2.0)
	add_child(_ghost)
...
func _process(_delta: float) -> void:
	...
	var tile: Vector2i = world.world_to_tile(get_global_mouse_position())
	_ghost.position = world.tile_to_world(tile)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("attack"):
		...
		if player == null or player.inventory.get(active_item, 0) <= 0:
			exit_build()
			return
		var tile: Vector2i = Game.world.world_to_tile(get_global_mouse_position())
